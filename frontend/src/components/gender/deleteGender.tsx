...
function DeleteGender({ open, close, id, onDelete }: { open: boolean, close: () => void, id: number, onDelete: () => void }) {
    // RESPONSE TYPE
    const responseType = 'Gender disable successfully';
    ...
    const submitInfo = async () => {
        try {
            // TRY DELETE THE ROL
            await fetchData(`/gender/disable/${id}`, setResponse);
        } catch (error) {
