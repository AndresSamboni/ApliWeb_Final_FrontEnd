...
// CREATE THE ExistsGender COMPONENT
function ExistsGender({ open, close, id, onExists }: { open: boolean, close: () => void, id: number, onExists: () => void }) {
    // RESPONSE TYPE
    ...
    const submitInfo = async () => {
        try {
            // TRY ENABLE ROLE
            await fetchData(`/gender/enable/${id}`, setResponse);
        } catch (error) {
    ...
    return (
        <ModalExists
            ...
            error={error}
            content={{
                title: "¿Estás seguro de habilitar el género?",
